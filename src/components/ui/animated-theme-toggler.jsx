...
export const AnimatedThemeToggler = ({
  ...
  if (!mounted) {
    return (
      <div
        className={cn("h-5 w-5 flex items-center justify-center", className)}
        aria-hidden="true"
      />
    )
  }
  ...
  return (
    ...
    >
      {isDark ? (
        <Sun className="h-5 w-5 text-yellow-500 dark:text-yellow-400 transition-all duration-200 group-hover:rotate-90" />
      ) : (
        <Moon className="h-5 w-5 text-neutral-700 dark:text-neutral-300 transition-all duration-200 group-hover:rotate-12" />
      )}
